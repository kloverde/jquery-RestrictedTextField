...
(function( $ ) {
   ...
   $.fn.restrictedTextField = function( options ) {
      ...
      function init() {
         ...
         // Input that's still being typed can fail 
         var negativeInt   = /^-$/,
             posNegFloat   = /^-?0?\.?$|^-?\d*\.\d*$/,
             positiveFloat = /^[\.\d]$|^\.$|^\d*\.$/,
             negativeFloat = /^0\.?$|^-\.?$|^-\d*\.$/;
         ...
         _addType( dest, "int",                     /^0$|^-?[1-9]\d*$/       , negativeInt );    // 0, positive integers, negative integers
         _addType( dest, "positiveInt",             /^0$|^[1-9]\d*$/         , null );           // 0, positive integers
         _addType( dest, "negativeInt",             /^0$|^-[1-9]\d*$/        , negativeInt );    // 0, negative integers
         ...
         _addType( dest, "strictFloat",             /^0$|^0?\.(0|\d*[1-9])$|^-0?\.(\d*[1-9])$|^-?[1-9]\d*\.\d+$|^-?[1-9]+\d*$/  , posNegFloat );    // 0, .0, 0.0, /positive/negative floating-point numbers, with or without a value to the left of the decimal point; positive and negative integers
